...
namespace RealifeGM.de.jojoa.player
{
	public class Playerlist : Script
	{
		...
		{
			API.triggerClientEventForAll("playerlist_join", player.socialClubName, player.name, ColorForPlayer(player),getID(player));
		}
		...
		public int getID(Client player) 
		{
            int i = 0;
            i =  mysql.MySQL_PlayerData.getInt(player, "id");
            return i;
		}
